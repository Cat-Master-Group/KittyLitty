...
if (typeof globalThis.appComponents.SignUp === "undefined") {
  globalThis.appComponents.SignUp = {
    ...
    validateUsername: function validateUsername() {
      ...
      const usernameErrorSpan = document.getElementById(
        "signup-username-error"
      );
      const usernameInput =
        usernameElement && usernameElement.value
    ...
    validatePassword: function validatePassword() {
      ...
      const validationResult =
        appComponents.Validate.signupPassword(passwordInput);
    ...
    validateCatName: function validateCatName() {
      ...
      const catNameInput =
        catNameElement && catNameElement.value
          ? catNameElement.value.trim()
          : "";
    ...
    validateGender: function validateGender() {
      ...
      const catGenderErrorSpan = document.getElementById(
        "signup-cat-gender-error"
      );
      const catGenderInput =
        catGenderElement.options[catGenderElement.selectedIndex].value;
      const validationResult =
        appComponents.Validate.isEmptyString(catGenderInput);
    ...
    validateAge: function validateAge() {
      ...
      const catAgeMonthElement = document.getElementById(
        "signup-cat-age-months"
      );
      ...
      if (
        appComponents.Validate.isEmptyString(catAgeYearInput) ||
        appComponents.Validate.isEmptyString(catAgeMonthInput)
      ) {
    ...
    validateBreed: function validateBreed() {
      ...
      const catBreedErrorSpan = document.getElementById(
        "signup-cat-breed-error"
      );
      const catBreedInput =
        catBreedElement && catBreedElement.value
          ? catBreedElement.value.trim()
          : "";
      const validationResult = appComponents.Validate.name(catBreedInput);

      if (validationResult && validationResult.valid === false) {
        if (validationResult.errorMessage) {
          catBreedErrorSpan.textContent = validationResult.errorMessage
            .replaceAll("name", "breed")
            .replaceAll("Name", "Breed");
        }
        return false;
      }
    ...
    validateAlteredStatus: function validateAlteredStatus() {
      ...
      const catAlteredErrorSpan = document.getElementById(
        "signup-cat-altered-error"
      );
      const catAlteredInput =
        catAlteredElement.options[catAlteredElement.selectedIndex].value;
      const validationResult =
        appComponents.Validate.isEmptyString(catAlteredInput);
    ...
    validateImage: function validateImage(index, required) {
      const catImageElement = document.getElementById(
        "signup-cat-image-" + index.toString()
      );
      const catImageErrorSpan = document.getElementById(
        "signup-cat-image-error-" + index.toString()
      );
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (data) => {
          ...
          const catGallery = [];
          catGallery.push(
            filterXSS(document.getElementById("signup-cat-image-1").value)
          );
          const imageUrl2 = filterXSS(
            document.getElementById("signup-cat-image-2").value
          );
          const imageUrl3 = filterXSS(
            document.getElementById("signup-cat-image-3").value
          );
          ...
          const isValid = appComponents.SignUp.validateSignup();
          console.log(isValid);
          if (isValid) {
            ...
            const requestConfig = {
              ...
              data: {
                ...
                userCat: {
                  catName: filterXSS(
                    document.getElementById("signup-cat-name").value
                  ),
                  catGender: filterXSS(
                    document.getElementById("signup-cat-gender").value
                  ),
                  catAge:
                    parseInt(
                      filterXSS(
                        document.getElementById("signup-cat-age-years").value
                      )
                    ) +
                    parseInt(
                      filterXSS(
                        document.getElementById("signup-cat-age-months").value
                      )
                    ),
                  catBreed: filterXSS(
                    document.getElementById("signup-cat-breed").value
                  ),
                  catIsAltered:
                    filterXSS(
                      document.getElementById("signup-cat-altered").value
                    ) === "true",
                  catGallery: catGallery,
                },
                userLocation: [long, lat],
                userBio: filterXSS(
                  document.getElementById("signup-profile-bio").value
                ),
              },
              error: (err) => {
